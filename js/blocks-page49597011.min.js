...
function t702_initPopup(recId) {
  let rec = document.getElementById("rec" + recId);
  if (!rec) return;
  let container = rec.querySelector(".t702");
  if (!container) return;
  rec.setAttribute("data-animationappear", "off");
  rec.setAttribute("data-popup-subscribe-inited", "y");
  rec.style.opacity = 1;
  let documentBody = document.body;
  let popup = rec.querySelector(".t-popup");
  let popupTooltipHook = popup.getAttribute("data-tooltip-hook");
  let analitics = popup.getAttribute("data-track-popup");
  let popupCloseBtn = popup.querySelector(".t-popup__close");
  let hrefs = rec.querySelectorAll('a[href*="#"]');
  let submitHref = rec.querySelector('.t-submit[href*="#"]');
  if (popupTooltipHook) {
    ...
    document.addEventListener("click", function (event) {
      ...
      let href = target.closest('a[href$="' + popupTooltipHook + '"]')
        ? target
        : !1;
      if (!href) return;
  ...
  popup.addEventListener("click", function (event) {
    let windowWithoutScrollBar = window.innerWidth - 17;
...
function t702_unlockScroll() {
  let documentBody = document.body;
  if (documentBody.classList.contains("t-body_scroll-locked")) {
    let bodyScrollTop = documentBody.getAttribute("data-popup-scrolltop");
    documentBody.classList.remove("t-body_scroll-locked");
...
function t702_showPopup(recId) {
  ...
  let screenMin = rec.getAttribute("data-screen-min");
  let screenMax = rec.getAttribute("data-screen-max");
  if (screenMin && windowWidth < parseInt(screenMin, 10)) return;
  if (screenMax && windowWidth > parseInt(screenMax, 10)) return;
  let popup = rec.querySelector(".t-popup");
  let popupTooltipHook = popup.getAttribute("data-tooltip-hook");
  let ranges = rec.querySelectorAll(".t-range");
  let documentBody = document.body;
  if (ranges.length) {
...
function t702_closePopup(recId) {
  ...
  let popup = rec.querySelector(".t-popup");
  let popupTooltipHook = popup.getAttribute("data-tooltip-hook");
  let popupAll = document.querySelectorAll(
  ...
  setTimeout(function () {
    let popupHide = document.querySelectorAll(".t-popup:not(.t-popup_show)");
    for (let i = 0; i < popupHide.length; i++) {
